...
export type MultiSelectItem<Id = any> = {
  ...
  disabled?: boolean;
  /** Whether this item acts as a "select all" toggle */
  isSelectAll?: boolean;
};

export type CarbonMultiSelectContext = {
  declareRef: (data: {
    key: "field" | "selection";
    ref: HTMLDivElement;
  }) => void;
...
type $Props<Item extends MultiSelectItem<any> = MultiSelectItem<any>> = {
   ...
   */
  sortItem?: ((a: Item, b: Item) => number) | (() => void);
   ...
   * @default []
   */
  sortedItems?: ReadonlyArray<Item & { checked: boolean }>;
